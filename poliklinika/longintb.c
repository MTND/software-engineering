...
int k = 0;
    ...
    if(*(read+i) != '\0')
        ...
        dummy = malloc(sizeof(list));
            if (*(read+i) >= 48 && *(read+i) < 58 ) {  dummy -> number = (int)*(read+i); dummy -> number = dummy -> number-48; ++sk;}  //printf("%c ", *(read+i) );
            else   { printf("klaida\n"); exit(1);}
...
char* printnumber(list *num){
    ...
    while (num!= NULL){
        ...
        num=num->next;
        dummy-> next = save;
        save = dummy;list *Read( char *read ){
  int sk=0;
  int n=0;
    list *head = NULL; list * curr1;
    list *curr = NULL;
...
skaicius = calloc( kiek , sizeof(char));
    ...
    *(skaicius+i) =  (char) 48 + dummy-> number;
            dummy = dummy-> next;
                    ++i;
        }
        realloc(skaicius, (i , sizeof(char)));
return skaicius;
    free(skaicius);
}
//**********************************************
